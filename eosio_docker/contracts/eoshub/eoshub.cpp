...
class [[eosio::contract]] eoshub : public eosio::contract {
  ...
  public:
    ...
    // handle_deposit notification _from_ the eoshub.token contract
    [[eosio::action]] void handle_deposit( name from, name to, asset quantity, std::string memo ) {
...
extern "C" {
    void apply(uint64_t receiver, uint64_t code, uint64_t action) {
        if(code==receiver)
        {
            ...
            execute_action( name(receiver), name(code), &eoshub::handle_deposit );
        }
    }
};
